...
int main()
{
	...
	if (c == "+")
	{
		if (s1.length() < s2.length()) s2[s2.length() - s1.length()] = '1', cout << s2;
		else if (s1.length() > s2.length()) s1[s1.length() - s2.length()] = '1', cout << s1;
		else if (s1.length() == s2.length()) s1[0] = '2', cout << s1;
	}
	else
	{
		s1.erase(s1.begin());
